...
public class SetPlayerState extends State {
	...
	private List<Integer> controllersHash;
	...
	public void render(SpriteBatch sb) {
		super.render(sb);
		sb.begin();
		font.draw(sb, "playerMap: " + playerMap + ", controllersHash: " + controllersHash, 0,
				SpaceRumble.HEIGHT * getZoomFactor());
		sb.end();
	...
	public boolean buttonDown(Controller controller, int buttonCode) {
		...
		for (int i = 1; i <= max; i++) {
			if (!playerMap.containsKey(i) && !controllersHash.contains(controller.hashCode())) {
				int A = XboxGamepad.A;
				int L1 = XboxGamepad.L1;
				int R1 = XboxGamepad.R1;
				Player player = new Player("" + i, A, L1, R1);
		...
		if (Input.Keys.ENTER == keycode) {
			...
			for (int i = 1; i <= max; i++) {
				if (!playerMap.containsKey(i) && !controllersHash.contains(1)) {
					Player player = new Player("" + i, Input.Keys.UP, Input.Keys.LEFT, Input.Keys.RIGHT);
					player.setInputId(1);
					playerMap.put(i, player);
